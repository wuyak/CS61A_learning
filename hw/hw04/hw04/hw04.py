...
def shuffle(s):
    ...
    s1 = s[:mid]
    s2 = s[mid:]
    interleaved_list = [ item for pair in zip(s1, s2) for item in pair]
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # You need to check if the end of recursion is balanced for back
    if is_planet(m):
        return True

    assert is_mobile(m), "must judge blance of a mobile or a planet"
    ...
    # you need to check the left or right is balanced too
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        return label(t)
    max_branch = max(branches(t),key=lambda branch: max_path_sum(branch))
    return label(t) + max_path_sum(max_branch)
