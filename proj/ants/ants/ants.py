...
class Ant(Insect):
    ...
    def double(self):
        ...
        # Don not Forget double only once !!!
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        ...
        while place:
            ant = place.ant
            if ant:
                self.apply_double(ant)
                if ant.is_container and ant.ant_contained:
                    self.apply_double(ant.ant_contained)
            place = place.exit

    def apply_double(self, ant):
...
class HarvesterAnt(Ant):
    ...
    def action(self, gamestate):
        """Produce 1 additional food for the colny.
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current_place.is_hive:
            if self.lower_bound <= atk_dist <= self.upper_bound:
...
"""
It is incorrect to use list_bees directly, because if the Place changes, list_bees will also change. You could use list(list_bees) for general cases, but you can use list_bees[:] specifically when working with lists.
"""
#        for bee in list_bees:
#            bee.reduce_health(total_damage)
