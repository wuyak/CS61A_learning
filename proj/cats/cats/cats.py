...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k < len(select):
        return select[k]
    return ''
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    min_len, max_len = min(t_len, s_len), max(t_len, s_len)
    ...
    for i in range(min_len):
        if typed_words[i] == source_words[i]:
            count_accuracy += 1
    return count_accuracy * 100 / t_len
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    selected_word, min_diff = min(
        ((word, diff_function(typed_word, word, limit)) for word in word_list),
        key=lambda x: x[1])
    return selected_word if min_diff <= limit else typed_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def compare_char(typed, source, sum_diff):
        if sum_diff > limit:
            return sum_diff
        ...
        if typed[0] != source[0]:
            sum_diff += 1
        return compare_char(typed[1:], source[1:], sum_diff)
    return compare_char(typed, source, abs(len(typed) - len(source)))
...
def minimum_mewtations(typed, source, limit):
    ...
    return min(insert, delete, substitute) + 1


"""
test case in problem EC forbidden call function , so we don't use min
    return min(insert, delete, substitute) + 1
"""
...
def final_diff(typed, source, limit):
    ...
    def manipulate(typed, source, sum_diff):
        if sum_diff > limit:
            return float('inf')
        if not typed or not source:
            return sum_diff + abs(len(typed) - len(source))
        if typed[0] == source[0]:
            return manipulate(typed[1:], source[1:], sum_diff)

        insert = manipulate(typed, source[1:], sum_diff + 1)
        delete = manipulate(typed[1:], source, sum_diff + 1)
        substitute = manipulate(typed[1:], source[1:], sum_diff + 1)

        return min(insert, delete, substitute)
    return manipulate(typed, source, 0)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = [[lst[i + 1] - lst[i] for i in range(len(lst) - 1)] for lst in tpp]
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    target_words = [[] for _ in player_indices]
    for i in word_indices:
        fastest_player = min(player_indices, key=lambda p, i=i: times[p][i])
        target_words[fastest_player].append(words[i])
    return target_words
    # END PROBLEM 10
